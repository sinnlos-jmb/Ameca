...
String periodo2  = request.getParameter ("periodo2") != null ?  request.getParameter ("periodo2") : "" ;
    ...
    else if (operacion.equals("ver_c"))  //ver las DDJJs de un comercio dada su id_comercio
        {
        out.println(HTML.getHead("liquidaciones", htm.getPeriodo_nostatic()));
...
//            out.println("<br><br>registros agregados: \n"+this.LiquiMes_masive(periodo, base_imponible, percepcion_iva, percepcion_iibb, compra_iva));
        switch (do_masive)
            {
            case "2": out.println("<br><br>registros modificados: \n"+this.LiquiMesIVA_update(periodo, base_imponible, percepcion_iva, compra_iva));
                    break;
            case "1": out.println("<br><br>registros agregados: \n"+this.LiquiMes_masive(periodo, base_imponible));  // bi es porcentaje gral
...
// Recibe  el id_comercio + periodo y devuelve una tabla de sus saldos DDJJ (IVA e IIBB) para el periodo 
    ...
    private String LiquiComercioTable(String id_comercio, String periodo) 
	{
        ...
        ResultSet rs = null;
        int i=0;
        String resul, nombre_establecimiento, direccion_establecimiento, query;
        float base_imponible=0f, debito_iva=0f, debito_iibb=0f, venta_total=0f, compra_iva=0f, credito_iva=0f, compra_total=0f, 
                percepcion_iva=0f, percepcion_iibb=0f, saldo_iva=0f, saldo_iibb=0f, alicuota_iva=0f, alicuota_iibb=0f,
...
// Recibe  el periodo y devuelve una tabla con los saldos DDJJ (IVA e IIBB) de los comercios para el periodo 
    ...
    private String LiquiMesTable(String periodo) 
	{
        ...
        int z=0, i=0, k=0;
        String resul;
        Float cmrc_bi=0.0f, cmrc_compra=0f, cmrc_percepcion_iva=0f, cmrc_percepcion_iibb=0f, cmrc_alicuota_iibb=0f, debito_iibb=0f,  saldo_iibb=0f, saldo_iva=0f;
        ...
        if(k>1)
            resul+="\n\t<tr> <td>"+cadenas[0][0]+"</td> <td>"+cadenas[0][1]+"</td> <td>"+cadenas[0][2]+", "+htm.getZona(enteros[0][5])+"</td> "+
               ...
               " \n\t <td></td>  </tr> \n "+
               "\n\t</table><br><br>Cantidad de Establecimientos: "+Integer.toString(k);
        else
...
// Recibe  el periodo y devuelve una tabla con los saldos DDJJ IVA de los comercios para el periodo 
	...
	String resul, row_span, zona, query, nombre_responsable, nro_cuit, direccion_establecimiento, id_establecimiento, id_comercio;
        ...
        int rows=0, int_zona, length_obs=0;
        try 
            {
            con=CX.getCx_pool();
        ...
        if (id_zona.equals(""))
        {
                ...
                File tempFile = new File("/usr/share/tomcat8-ameca/ameca/Reportes/"+htm.getPeriodo_year()+"/liqui_iva_"+periodo+".xls");
        ...
        resul+="</tr>"+
                 ...
                 "</form> \n"+
                 "<br><br>"+
                    "<table class='grupos' width='1910px'>\n\t<tr>\n\t\t"+
                ...
                if (i<0)    //entra la primera vez y cuando se termino` con los establecimientos de un comercio
                    {
                     ...
                     if (i==1)  // unico establecimiento del comercio     /ameca/comercios?operacion=detalle&nro_cuit=23-24963650-9&id_comercio=326
                     ...
                     else   // al menos dos, imprimo el primer establecimiento de la serie
                        {
                         ...
                         if (length_obs>0)
                                resul+= "<td class='t2' align='right'> <a href=\"#\" onClick=\"MyWindow=window.open('/ameca/establecimientos?operacion=notes&nombre_responsable_establecimiento="+URLEncoder.encode(nombre_responsable)+"&direccion_establecimiento="+URLEncoder.encode(direccion_establecimiento)+"&nro_cuit="+nro_cuit+"&id_establecimiento="+id_establecimiento+"','MyWindow','width=600,height=300'); return false;\"><img src=\"/ameca/imgs/notepad.png\" style=\"width:32px;height:32px;\" onmouseover=\"this.style.cursor='pointer'\">\n</a></td>  \n\t</tr>";
                         else
                                resul+= "<td class='t2' align='right'> <a href=\"#\" onClick=\"MyWindow=window.open('/ameca/establecimientos?operacion=notes&nombre_responsable_establecimiento="+URLEncoder.encode(nombre_responsable)+"&direccion_establecimiento="+URLEncoder.encode(direccion_establecimiento)+"&nro_cuit="+nro_cuit+"&id_establecimiento="+id_establecimiento+"','MyWindow','width=600,height=300'); return false;\"><img src=\"/ameca/imgs/notepad_gris.png\" style=\"width:32px;height:32px;\" onmouseover=\"this.style.cursor='pointer'\">\n</a></td>  \n\t</tr>";
...
// cookie y llave para habilitar esta seccion (no cookie .:. llave que pide pwd con javascript prompt x ahora y despues pop up)
            ...
            if (pwd_cookie.equals("yes")  || pwd_cookie.equals("yes_admin"))
                    {
                     ...
                     if(rows<1 )
                              ...
                              {
                                  if(id_zona.equals(""))
                                      resul+="<br><br><br><br>Actualizacion de todos los establecimientos. Ingrese porcentajes.<br>\n"+
                                              ...
                                              "</tr>\n</table>\n"+
                                              "</form>";
                                  resul+="<br><br><br> registros: "+Integer.toString(rows);
                              }
...
//    Recibe  el periodo y devuelve una tabla con los saldos DDJJ IVA de los comercios para el periodo 
    private String LiquiMesIIBBTable(String periodo, String zona) 
	{
        ...
        int i=-1, id_zona=0, condicion_iibb=0, rows=0, length_obs=0 ;
        String resul, row_span, query, nom_zona, id_establecimiento, direccion_establecimiento="", nombre_responsable="", nro_cuit="", id_comercio="";
        ...
        if (zona.equals(""))
            {
                ...
                File tempFile = new File("/usr/share/tomcat8-ameca/ameca/Reportes/"+htm.getPeriodo_year()+"/liqui_iibb_"+periodo+".xls");
        ...
        resul+="</tr>"+
                 ...
                 "</form> \n"+
                 "<br><br>"+
                 "<table class='grupos' width='1420px'>\n\t<tr>\n\t\t"+
...
//                     row_span=rs.getString(9);
                     ...
                     if (i==1)  // unico establecimiento del comercio
                        {
                         ...
                         if (length_obs>0)
                                resul+= "<td class='t11'>"+String.format(Locale.GERMAN, "%,.2f",cmrc_percepcion)+"</td> <td class='t11'>"+String.format(Locale.GERMAN, "%,.2f",cmrc_saldo)+"</td> <td class='t11' align='right'> <a href=\"#\" onClick=\"MyWindow=window.open('/ameca/establecimientos?operacion=notes&nombre_responsable_establecimiento="+URLEncoder.encode(nombre_responsable)+"&direccion_establecimiento="+URLEncoder.encode(direccion_establecimiento)+"&nro_cuit="+nro_cuit+"&id_establecimiento="+id_establecimiento+"','MyWindow','width=600,height=300'); return false;\"><img src=\"/ameca/imgs/notepad.png\" style=\"width:32px;height:32px;\"  onmouseover=\"this.style.cursor='pointer'\">\n</a></td>\n\t</tr>\n ";
                         else
                                resul+= "<td class='t11'>"+String.format(Locale.GERMAN, "%,.2f",cmrc_percepcion)+"</td> <td class='t11'>"+String.format(Locale.GERMAN, "%,.2f",cmrc_saldo)+"</td> <td class='t11' align='right'> <a href=\"#\" onClick=\"MyWindow=window.open('/ameca/establecimientos?operacion=notes&nombre_responsable_establecimiento="+URLEncoder.encode(nombre_responsable)+"&direccion_establecimiento="+URLEncoder.encode(direccion_establecimiento)+"&nro_cuit="+nro_cuit+"&id_establecimiento="+id_establecimiento+"','MyWindow','width=600,height=300'); return false;\"><img src=\"/ameca/imgs/notepad_gris.png\" style=\"width:32px;height:32px;\"  onmouseover=\"this.style.cursor='pointer'\">\n</a></td>\n\t</tr>\n ";
 //                           "\n\t</tr>\n <tr> <td colspan='8' bgcolor='#E8E7C1' height='1px'> </td></tr>";
    ...
    // Esta fumcion esta` repetida, es la misma para iibb e iva, ya que actualiza todos los establecimientos activos de EstablecimientoLiquiMes.
    ...
    private String LiquiMesIIBB_update(String periodo, String base_imponible, String percepcion_iibb) 
	{
     ...
     // insert masivo de los establecimientos en EstablecimientoLiquiMes para nuevo periodo sin actividades todavia.
